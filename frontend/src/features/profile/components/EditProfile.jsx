...
export default function EditProfile() {
  ...
  useEffect(() => {
    if (userProfile) {
      ...
      if (typeof userProfile.socials === "string") {
        try {
          userSocials = JSON.parse(userProfile.socials);
        } catch (e) {
          console.error("Invalid socials JSON");
        }
  ...
  const getInitials = (name, surname) => {
    ...
    const lastNameInitial = surname ? surname.charAt(0) : "";
    ...
    <>
      <div className="w-full max-w-4xl mx-auto py-8 text-white">
        <header className="mb-10">
          <h1 className="text-3xl font-bold tracking-tight" id="main-heading">
            Profile Settings
          </h1>
        ...
        <form onSubmit={handleSubmit} className="space-y-12" aria-labelledby="main-heading">
          <section aria-labelledby="picture-heading">
            <h2 id="picture-heading" className="text-xl font-semibold text-white">
              Profile Picture
            </h2>
            <div className="mt-4 flex items-center gap-6">
              <Avatar className="w-28 h-28 border-2 border-neutral-700">
                <AvatarImage src={preview} alt="Current profile picture" />
                <AvatarFallback className="text-4xl bg-neutral-800 text-neutral-300">{getInitials(formData.name, formData.surname)}</AvatarFallback>
              </Avatar>
          ...
          <section aria-labelledby="personal-info-heading">
            <h2 id="personal-info-heading" className="text-xl font-semibold text-white">
            ...
            <div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-8">
              <div>
                ...
                <Input id="name" name="name" value={formData.name} onChange={handleChange} className="mt-2 bg-neutral-800 border-neutral-700" />
              </div>
              <div>
                <Label htmlFor="surname">Last Name</Label>
                <Input id="surname" name="surname" value={formData.surname} onChange={handleChange} className="mt-2 bg-neutral-800 border-neutral-700" />
              </div>
              <div>
                <Label htmlFor="birthdate">Birthdate</Label>
                <Popover>
                  <PopoverTrigger asChild>
                    <Button variant={"outline"} id="birthdate" className={cn("w-full justify-start text-left font-normal mt-2 bg-neutral-800 border-neutral-700 hover:bg-neutral-700", !formData.birthdate && "text-neutral-400")}>
                  ...
                  <PopoverContent className="w-auto p-0" align="start">
                    <Calendar
                      className={cn("bg-neutral-800 text-neutral-300 border-neutral-700")}
                      mode="single"
                      selected={formData.birthdate}
                      onSelect={handleDateChange}
                      initialFocus
                      captionLayout="dropdown"
                      fromYear={new Date().getFullYear() - 120} // Allow selection from 120 years ago
                      toYear={new Date().getFullYear()} // Allow selection up to the current year
                      toDate={new Date()} // Disable all future dates
                    />
                  </PopoverContent>
                </Popover>
              </div>
              <div>
                ...
                <Input id="country" name="country" value={formData.country} onChange={handleChange} className="mt-2 bg-neutral-800 border-neutral-700" />
              </div>
            </div>
          </section>
          ...
          <section aria-labelledby="public-profile-heading">
            <h2 id="public-profile-heading" className="text-xl font-semibold text-white">
              Public Profile
            </h2>
            ...
            <div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-8">
              <div>
                ...
                <Input id="username" name="username" value={formData.username} onChange={handleChange} className="mt-2 bg-neutral-800 border-neutral-700" />
              </div>
              <div className="md:col-span-2">
                <Label htmlFor="info">Bio</Label>
                <Textarea id="info" name="info" value={formData.info} onChange={handleChange} placeholder="Tell us about yourself." className="mt-2 bg-neutral-800 border-neutral-700" />
              </div>
            </div>
          </section>
          ...
          <section aria-labelledby="account-info-heading">
            <h2 id="account-info-heading" className="text-xl font-semibold text-white">
              Account Information
            </h2>
            <div className="mt-6 max-w-md space-y-8">
              ...
              <div>
                ...
                <Input id="phoneNumber" type="tel" name="phoneNumber" value={formData.phoneNumber} onChange={handleChange} className="mt-2 bg-neutral-800 border-neutral-700" />
              </div>
            </div>
          </section>
          ...
          <section aria-labelledby="social-links-heading">
            <h2 id="social-links-heading" className="text-xl font-semibold text-white">
              Social Links
            </h2>
          ...
          <div className="flex justify-end gap-4 pt-4">
            <Button type="button" variant="outline" className="text-white border-neutral-700 hover:bg-neutral-800 hover:text-white">
              Cancel
            </Button>
      ...
      <Dialog open={isUploadDialogOpen} onOpenChange={setIsUploadDialogOpen}>
        <DialogContent className="bg-neutral-900 border-neutral-800 text-white">
          <DialogHeader>
            ...
            <DialogDescription>Drag and drop your image, or click to browse.</DialogDescription>
          </DialogHeader>
          <div role="button" tabIndex={0} onKeyDown={(e) => (e.key === "Enter" || e.key === " ") && fileInputRef.current.click()} onDragEnter={handleDragEnter} onDragLeave={handleDragLeave} onDragOver={handleDragOver} onDrop={handleDrop} onClick={() => fileInputRef.current.click()} className={`mt-4 w-full h-64 border-2 border-dashed rounded-lg flex flex-col justify-center items-center cursor-pointer transition-colors ${isDragging ? "border-blue-500 bg-blue-900/20" : "border-neutral-700 hover:border-neutral-600"}`} aria-label="Image upload zone">
            ...
            {uploadPictureMutation.isPending ? (
              ...
              <div className="text-center pointer-events-none">
                <Camera className="mx-auto h-10 w-10 text-neutral-500" />
                <p className="mt-2 text-sm text-neutral-400">
                  <span className="font-semibold text-blue-500">Click to upload</span> or drag and drop
                </p>
                <p className="text-xs text-neutral-500 mt-1">PNG, JPG, or GIF</p>
              </div>
            )}
          </div>
        </DialogContent>
