      ...
      <div className="divide-y divide-gray-800">
        ...
        <InfoSection title="Achievements" icon={Trophy}>
          {user.achievements && user.achievements.length > 0 ? (
            ...
            <Placeholder text="No achievements yet." />
          )}
        </InfoSection>

        {/* === STATS SEKTION === */}
        <InfoSection title="Statistics" icon={Trophy}>
          {user.statistics && user.statistics.length > 0 ? (
            <ul className="list-disc pl-5 space-y-2">
              {user.statistics.map((statistic, index) => (
                <li key={index} className="text-gray-300">
                  {statistic}
                </li>
              ))}
            ...
            <Placeholder text="No statistics available." />
          )}
        </InfoSection>
      </div>
    </div>
